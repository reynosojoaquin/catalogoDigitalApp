...
class RolController {
  ...
  static async create(req, res) {
    ...
    try {
      const entityCreated = await RolService.create(newRol);
  ...
  static async getAll(req, res) {
    ...
    try {
      const { rows, count, total } = await RolService.getAll({
        criterions: {
          where,
          limit,
          offset,
  ...
  static async getById(req, res) {
    try {
      const user = await RolService.getById(req.params.id);
  ...
  static async update(req, res) {
    ...
    try {
      const updateRol = await RolService.update(id, rol);

      return res.json(Response.get('User Updated', updateRol));
    } catch (error) {
      return res.json(Response.get('Something goes wrong', error, 500));
    }
  }
